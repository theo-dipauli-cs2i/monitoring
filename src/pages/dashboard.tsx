                    ...
                    {(chartVisibility.clientChart || chartVisibility.commandeChart || chartVisibility.chiffreAffaireChart) && (
                        <Paper sx={{ p: { xs: 1, sm: 2 } }}>
                            <Box sx={{
                                ...
                                {chartVisibility.clientChart && <Box sx={{ display: 'flex', flex: 1, minWidth: 0, width: '100%' }}><ClientChart /></Box>}
                                {chartVisibility.commandeChart && <Box sx={{ display: 'flex', flex: 1, minWidth: 0, width: '100%' }}><CommandeChart /></Box>}
                                {chartVisibility.chiffreAffaireChart && <Box sx={{ display: 'flex', flex: 1, minWidth: 0, width: '100%' }}><ChiffreAffaireChart /></Box>}
                            </Box>
                        </Paper>
                    )}
                    {(chartVisibility.httpStatusChart || chartVisibility.logsList) && (
                        <Paper sx={{ p: { xs: 1, sm: 2 }, mt: 2 }}>
                            {chartVisibility.httpStatusChart && <Box sx={{ overflowX: 'auto' }}><HttpStatusChart /></Box>}
                            {chartVisibility.logsList && <Box sx={{ overflowX: 'auto' }}><LogsList /></Box>}
                        </Paper>
                    )}
                </Grid>
